...
func (s *server) exec(p *struct{ Action, Target, Command, Service, OS, Arch, Binary string }) string {
	switch p.Action {
	case "build":
		if !isValidBinary(p.Binary) {
			return "error: invalid binary name: " + esc(p.Binary)
		}
		r, _ := exec.Command("go", "build", "-o", p.Binary, ".").CombinedOutput()
		return string(r)
	case "ssh":
		if !isValidTarget(p.Target) {
			return "error: invalid target: " + esc(p.Target)
		}
		if !isValidRemoteCommand(p.Command) {
			return "error: invalid command: " + esc(p.Command)
		}
...
// isValidTarget validates an SSH target: user@hostname or hostname.
// Rejects leading dash (SSH option injection) and shell metacharacters.
...
// isValidRemoteCommand validates a command string for remote SSH execution.
// Allows only alphanumerics, basic punctuation, and common safe characters.
func isValidRemoteCommand(s string) bool {
	if s == "" {
		return false
	}
	for _, r := range s {
...
// isValidBinary validates a binary filename: no path separators, no shell chars.
