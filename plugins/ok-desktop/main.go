...
func (s *mcpServer) exec(name string, args json.RawMessage) (string, error) {
	...
	case "focus-window":
		return ps(fmt.Sprintf("$h=(Get-Process | Where-Object {$_.MainWindowTitle -match %s}).MainWindowHandle; Add-Type -Name W -Member '[DllImport(\"user32.dll\")]public static extern bool SetForegroundWindow(IntPtr hWnd)'; [W]::SetForegroundWindow($h)", escPS(p.Target)))
	case "notify":
