...
// across turns and is canceled only when the controller closes (or kill_shell is
...
// context (canceled by Close). sink receives job-lifecycle notices; pass the
...
// canceled, failed on any other error, else done.
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				...
				var recErr error
				if e, ok := r.(error); ok {
					recErr = fmt.Errorf("panic: %w", e)
				} else {
					recErr = fmt.Errorf("panic: %v", r)
				}
				m.recordCompletion(id, kind, label, st, recErr)
				close(j.done)
				m.wg.Done()
			}
		}()
		...
		// signaling done, so a Wait that unblocks on j.done sees the note already
...
// empty) reach a terminal state, or ctx is canceled, or timeoutSec elapses
...
func (m *Manager) Wait(ctx context.Context, ids []string, timeoutSec int) []Result {
	...
	// Wait for all targets concurrently so one hung job doesn't starve the rest.
			...
			defer func() {
				if r := recover(); r != nil {
					m.sink.Emit(&event.Event{
						...
						Text:  fmt.Sprintf("jobs: panic in Wait goroutine: %v", r),
					})
					done <- struct{}{}
				}
			}()
...
func (m *Manager) resolve(ids []string) []*Job {
	...
	if len(ids) == 0 {
		...
		// Purge terminal jobs older than the 500 most recent entries so the
		// map and order slice don't grow unbounded over many background tasks.
		if len(m.order) > 500 {
			keep := make(map[string]bool)
			for _, id := range m.order[len(m.order)-250:] {
				keep[id] = true
			}
			for _, id := range m.order {
				j := m.jobs[id]
				j.mu.Lock()
				terminal := j.status == Done || j.status == Failed || j.status == Killed
				j.mu.Unlock()
				if terminal && !keep[id] {
					delete(m.jobs, id)
				}
			}
			m.order = m.order[len(m.order)-250:]
		}
		return out
	}
	for _, id := range ids {
