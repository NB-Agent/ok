...
func (m *chatTUI) fileItems(token string) []compItem {
	...
	// Collect matching entries first, capped at maxCompItems, THEN sort —
	// so a 100K-file directory doesn't freeze the UI on every keystroke.
	// os.ReadDir is already name-sorted on most OSes, so the sort here is
	// only to promote directories above files within the displayed subset.
	showHidden := strings.HasPrefix(frag, ".")
	items := make([]compItem, 0, len(entries))
	...
	sort.SliceStable(items, func(i, j int) bool {
		return items[i].descend && !items[j].descend
	})
