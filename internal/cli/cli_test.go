...
func TestGroupByFamily(t *testing.T) {
	...
	if got := order; !reflect.DeepEqual(got, []string{"deepseek", "mimo", "claude", "gemini-flash", "gemini-pro"}) {
		t.Fatalf("family order = %v, want [deepseek mimo claude]", got)
	}
	...
	if got := members["claude"]; !reflect.DeepEqual(got, []int{4, 5}) {
		t.Errorf("claude members = %v, want [4 5]", got)
	}
	if info["deepseek"].name != "DeepSeek" || info["mimo"].name != "MiMo (Xiaomi)" || info["claude"].name != "Claude (Anthropic)" {
		t.Errorf("display names = %q / %q / %q", info["deepseek"].name, info["mimo"].name, info["claude"].name)
	}
}
