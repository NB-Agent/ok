...
func (m *chatTUI) skillList() {
	skills := m.skills
	if len(skills) == 0 {
		m.notice("no skills found. Add SKILL.md / <name>.md under .ok/skills (project) or ~/.ok/skills (global); .agents/.agent/.claude skills dirs also work. Invoke with /<name> or run_skill.")
		return
	}
...
func (m *chatTUI) skillPaths() {
	...
	b.WriteString(dim("  project > custom > global > builtin; add custom roots via [skills] paths in ok.toml"))
	m.notice(b.String())
}
...
func (m *chatTUI) hooksList(cwd string) {
	...
	if hook.ProjectDefinesHooks(cwd) && !trusted {
		...
		fmt.Fprintf(&b, "%s", dim(fmt.Sprintf("  project trusted: %v · config: project .ok/settings.json + global ~/.ok/settings.json", trusted)))
	}
	m.notice(b.String())
}
