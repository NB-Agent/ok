...
// setupConfig runs the configuration wizard (the `ok setup` command),
// writing ok.toml (+ .env). Project memory is a separate concern — the
...
// initHint handles `ok init`. Unlike a config scaffold, project memory is
...
// (and to `ok setup` for config) so the verb isn't a dead end.
func initHint() int {
	fmt.Println(i18n.M.InitHint)
	return 0
}

// interactiveSetup runs the setup wizard, then writes ok.toml (plus .env for any
...
func interactiveSetup(path string) int {
	...
	// Now that the catalog matches the user's choice, show the welcome banner
	// in their language before any substantive prompt.
	fmt.Println()
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("ok")),
		"",
...
// label is bilingual because we don't yet know which catalog to trust.
...
// edit ok.toml afterward — keeping first-run a single decision instead of two.
...
// not exist in config because users editing ok.toml deal with SKU names
...
func familyOf(name string) providerFamily {
	...
	case strings.HasPrefix(name, "claude"):
		return providerFamily{key: "claude", name: "Claude (Anthropic)", desc: "industry-leading coding model"}
	default:
...
// are appended — so re-running `ok setup` with a corrected key replaces the
...
//
// SAFETY: os.Setenv is not goroutine-safe in Go. This function MUST only be
// called from the main goroutine during CLI startup (setup/init path), before
// any other goroutine that calls os.Getenv/os.Setenv is spawned.
