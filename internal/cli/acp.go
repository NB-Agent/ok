...
import (
	...
	"github.com/NB-Agent/ok/internal/acp"
...
// acpCommand runs OK as an Agent Client Protocol agent: a stdio JSON-RPC
...
func acpCommand(args []string, version string) int {
	...
	ctx, stop := signal.NotifyContext(context.Background(), os.Interrupt) // CLI entry point — no parent context
	defer stop()

	factory := &acpFactory{cfg: cfg, model: modelName}
	info := acp.AgentInfo{Name: "ok", Version: version}
...
func (f *acpFactory) NewSession(ctx context.Context, p acp.SessionParams) (*control.Controller, error) {
	...
	policy := permission.NewDenyPolicy(cfg.ModeDeny())
	headlessGate := permission.NewGate(policy, nil)
	reg.Add(agent.NewTaskTool(execProv, entry.Price, reg, maxSteps,
		entry.ContextWindow, cfg.Agent.Temperature, config.ArchiveDir(), "", headlessGate, "", nil, nil, 0))
	...
	if pm := cfg.Agent.PlannerModel; pm != "" {
		...
		if pe.Model != entry.Model {
			...
			runner = agent.NewCoordinator(plannerProv, plannerSess, pe.Price, executor, entry.Model, cfg.Agent.Temperature, p.Sink)
