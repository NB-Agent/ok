...
// isHTTPS reports whether url uses the HTTPS scheme, or is a localhost URL
// (127.0.0.1, ::1, localhost) that never leaves the machine.
func isHTTPS(url string) bool {
	lower := strings.ToLower(url)
	if strings.HasPrefix(lower, "https://") {
		return true
	}
	// Allow localhost for testing/local mock servers.
...
func Fetch(ctx context.Context, url, apiKey string) (*Balance, error) {
	...
	// Enforce HTTPS when sending an API key to prevent credential leakage
	// over plaintext HTTP. Localhost URLs are allowed for testing/mocking.
	if apiKey != "" && !isHTTPS(url) {
		return nil, fmt.Errorf("balance: refusing to send API key over non-HTTPS URL: %q", url)
	}
	...
	defer log.Close("balance response", resp.Body)
	body, err := io.ReadAll(io.LimitReader(resp.Body, 1<<16))
	if err != nil {
		return nil, fmt.Errorf("balance: read body: %w", err)
	}
