...
// Trust gates project hooks. A project's .ok/settings.json can run
...
// root. The trust flag lives in user-global state (~/.ok/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.ok/settings.json) are the user's own and always run.
...
func readTrust(homeDir string) trustFile {
	...
	if err := json.Unmarshal(b, &tf); err != nil {
		fmt.Fprintf(os.Stderr, "hook: malformed trust file at %s: %v\n", TrustPath(homeDir), err)
		return tf
	}
	return tf
}
