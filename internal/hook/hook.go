...
// (.ok/settings.json, only when the project is trusted) and a global
// (~/.ok/settings.json) file. A hook's exit
...
type ResolvedHook struct {
	...
	matchRe *regexp.Regexp // precompiled Match regex (nil = match all)
}

const (
	maxBlockingTimeout = 60 * time.Second  // PreToolUse, UserPromptSubmit
	maxAsyncTimeout    = 300 * time.Second // PostToolUse, Stop
)
...
func (h ResolvedHook) timeout() time.Duration {
	...
	if h.Timeout > 0 {
		...
		d = defaultTimeout(h.Event)
	}
	// Clamp to a safe max so a misconfigured hook can't hang the agent.
	if d > maxAsyncTimeout {
		d = maxAsyncTimeout
	}
	if (h.Event == PreToolUse || h.Event == UserPromptSubmit) && d > maxBlockingTimeout {
		d = maxBlockingTimeout
	}
	return d
}
	...
	for _, event := range Events {
		for _, cfg := range s.Hooks[event] {
			...
			*out = append(*out, ResolvedHook{
				...
				Scope:      scope,
				Source:     source,
				matchRe:    compileMatch(cfg.Match),
			})
		}
	}
}

// compileMatch pre-compiles a hook's match regex, returning nil for "match
// all" patterns so MatchesTool can use an atomic nil check.
func compileMatch(m string) *regexp.Regexp {
	if m == "" || m == "*" {
		return nil
	}
	re, err := regexp.Compile("^(?:" + m + ")$")
	if err != nil {
		return nil // malformed regex: never fires (safer than firing on everything)
	}
	return re
}
...
func MatchesTool(h ResolvedHook, toolName string) bool {
	...
	if h.matchRe != nil {
		return h.matchRe.MatchString(toolName)
	}
	// Slow path: hook was constructed without precompilation (e.g. in tests).
	m := h.Match
	if m == "" || m == "*" {
		return true
	}
...
func Run(ctx context.Context, payload Payload, hooks []ResolvedHook, spawner Spawner) Report {
	...
	if err != nil {
		stdinBytes = []byte("{}") // degraded: hooks receive empty payload on marshal failure
	}
...
func DefaultSpawner(ctx context.Context, in SpawnInput) SpawnResult {
	...
	cmd := winhide.CommandContext(cctx, name, args...)
	cmd.Dir = in.Cwd
