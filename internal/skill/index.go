...
const indexHeader = "# Skills — playbooks you can invoke\n\n" +
	"Call with run_skill({name:\"<id>\",arguments:\"<task>\"}) or /<name>. [🧬 subagent]=isolated subagent (context-heavy, returns final answer only); untagged=inline (body folded into this turn)."
...
func indexLine(sk Skill) string {
	...
	max := 80 - len([]rune(sk.Name)) - len([]rune(tag))
