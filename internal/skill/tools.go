...
func (*runSkillTool) Description() string {
	return "Invoke a skill from the Skills index. Pass `name` as the bare identifier (e.g. 'review'), " +
		"without the `[🧬 subagent]` tag. Subagent-tagged skills run isolated — only the final answer returns. " +
		"Untagged skills inline the body as a tool result to follow."
}
...
"type":"object",
"properties":{
  "name":{"type":"string","description":"Skill identifier as it appears in the pinned Skills index (e.g. 'review'). Case-sensitive. Just the identifier, not the [🧬 subagent] tag."},
...
// --- dedicated subagent wrappers (review / security_review) ---
...
func BuiltinSubagentTools(store *Store, runner SubagentRunner) []tool.Tool {
	specs := []struct {
		...
		{"review", "review",
			"Review current branch diff in an isolated subagent — flags correctness, security, missing tests, hidden behavior per file:line. Read-only. Use before PRs or after multi-step edits.",
			"What to focus the review on (e.g. 'focus on the auth changes' or 'general'). The subagent reads the diff itself."},
		{"security_review", "security-review",
			"Security review of current branch diff in an isolated subagent — injection, authz, secrets, deserialization, path-traversal, crypto, severity-tagged. Read-only. Use for changes touching auth, input parsing, file IO, or external requests.",
...
func (t *installSkillTool) Description() string {
	scope := "'global' writes to ~/.ok/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default, this workspace) or 'global' (all projects)."
	}
	return "Create a reusable skill playbook. Invocable immediately via run_skill (or /<name>); " +
		"appears in Skills index next launch. Scope: " + scope
}
...
func (t *installSkillTool) Execute(_ context.Context, args json.RawMessage) (string, error) {
	...
	if err != nil {
		// Roll back: remove the file since we can't report success.
		os.Remove(path)
