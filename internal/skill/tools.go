...
func (*runSkillTool) Description() string {
	return "Invoke a playbook from the Skills index pinned in the system prompt. Pass `name` as the BARE identifier (e.g. 'review'), NOT the `[🧬 subagent]` tag that follows it. `[🧬 subagent]` skills spawn an isolated subagent — only the final distilled answer returns. Untagged skills are inlined: the body becomes a tool result you read and follow."
}
...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to ~/.ok/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.ok/skills/ (this workspace only); 'global' writes to ~/.ok/skills/ (every project)."
	}
