...
func TestRunSkillInline(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".ok/skills/note.md", "---\ndescription: take a note\n---\nDo the thing.")
...
func TestRunSkillSubagentNeedsRunner(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".ok/skills/dig.md", "---\ndescription: dig\nrunAs: subagent\n---\nbody")
...
func TestBuiltinSubagentToolsRunner(t *testing.T) {
	...
	if _, err := reviewTool.Execute(context.Background(), json.RawMessage(`{"task":"check the diff"}`)); err != nil {
		t.Fatalf("execute: %v", err)
	}
	if ran != "review:check the diff" {
