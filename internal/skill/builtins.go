...
// Built-in skills ship with OK and back the dedicated subagent tools
// (review / security_review) plus the inline `test`
// playbook. The explore/research tools are superseded by task() with tools
// whitelist. A user/project file with the same name overrides the built-in (see
...
// builtinTestBody is a prompt template with shell-command examples — no SQL involved.
// The ok-verify SQL-injection analyzer may flag the string concatenation here as a false positive.
...
How to operate:
1. Check for an existing memory doc first: list the project root and look for OK.md / AGENTS.md / CLAUDE.md. If one exists, read it and IMPROVE it in place (fix stale facts, fill gaps) — write back to that same filename, don't clobber it wholesale or create a second file.
...
const builtinArchReviewBody = `You are an architecture-review subagent. Analyse a Go package's dependency graph and structure.

How to operate:
- Use grep to map imports (find all "github.com/NB-Agent/ok/internal/..." imports across the codebase).
- Use read_file to inspect the package's main file(s) and exported API.
- Trace dependencies: what does this package import, and what imports it?
- Build a dependency graph in your head. Check for cycles, god packages (>10 imports), high coupling (>5 inbound deps on a single file).

Report:
1. Package dependencies: imports X → used by Y.
2. Coupling hotspots: which packages are imported by many others?
3. God-package detection: a single package with >10 concern responsibilities.
4. Circular dependencies: any cycle? (report the exact cycle path).`

const builtinHealthCheckBody = `You are a health-check subagent. Run a full-dimensional health scan of the project.

Passes to run (each with bash):
1. Build: ` + "`go build ./...`" + ` — passes if zero exit + no stderr errors.
2. Test: ` + "`go test ./...`" + ` — passes if zero failures.
3. Vet: ` + "`go vet ./...`" + ` — passes if zero output.
4. Lint: run available linters (staticcheck, golangci-lint) if installed; skip gracefully if not.
5. Dead code: scan for unused exports, empty files, import cycles.
6. Dependencies: check go.mod for outdated or unused indirects.

Report a scorecard:
- Each pass: ✅ pass / ❌ fail / ⚠️ skipped (with reason).
- Overall health: 🟢 all green / 🟡 warnings / 🔴 failures.
- One-line recommendation: what to fix first.`

const builtinAutoFixBody = `You are an auto-fix subagent. Diagnose, fix, verify, and retry on failure.

Loop (max 3 attempts):
1. Run the failing command (the 'task' prompt tells you which — usually build or test).
2. Parse the error output. Identify the root cause (file, line, error message).
3. Apply the minimum fix (edit_file or write_file). Do NOT refactor or "while we're here".
4. Re-run the command. If it passes → done. If the SAME error persists after 2 attempts → stop and report. If a NEW error appears → that's progress, fix it next.
5. If you reach attempt 3 without success → stop and report what you tried.

Never retry the same approach twice. After a fix succeeds, emit a one-line summary of what changed.`
...
func builtinSkills() []Skill {
	return []Skill{
		...
		{
			Name:        "arch-review",
			Description: "Go architecture review — package dependency graph, coupling heatmap, god-package detection, circular dependency check. Subagent: read-only, returns one structured report per target package.",
			Body:        builtinArchReviewBody,
			Scope:       ScopeBuiltin,
			Path:        "(builtin)",
			RunAs:       RunSubagent,
		},
		{
			Name:        "health-check",
			Description: "Full-dimensional project health scan — build, test, dependency audit, dead code, lint, security. Subagent: runs multiple diagnostic passes and returns a unified scorecard.",
			Body:        builtinHealthCheckBody,
			Scope:       ScopeBuiltin,
			Path:        "(builtin)",
			RunAs:       RunSubagent,
		},
		{
			Name:        "auto-fix",
			Description: "Auto-fix pipeline — diagnose build/test failures, propose+apply fixes, verify, retry up to 3 attempts. Subagent: self-contained fix loop, returns final status + what changed.",
			Body:        builtinAutoFixBody,
			Scope:       ScopeBuiltin,
			Path:        "(builtin)",
			RunAs:       RunSubagent,
		},
