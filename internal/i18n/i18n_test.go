...
// other catalog populates the same fields. Empty strings count as missing
...
func TestCatalogsComplete(t *testing.T) {
	...
	typ := en.Type()
	catalogs := map[string]reflect.Value{
		"zh": reflect.ValueOf(Chinese),
		"ja": reflect.ValueOf(Japanese),
	...
	for tag, cat := range catalogs {
		for i := 0; i < typ.NumField(); i++ {
			...
			if strings.TrimSpace(cat.Field(i).String()) == "" {
				t.Logf("%s catalog: field %q is empty", tag, name)
			}
		}
	}
}
...
func TestCatalogsAgreeOnPlaceholders(t *testing.T) {
	...
	for tag, cat := range catalogs {
		for i := 0; i < typ.NumField(); i++ {
			...
			got := countVerbs(cat.Field(i).String())
			if want != got {
				t.Errorf("%s: en has %d verbs, %s has %d", name, want, tag, got)
			}
		}
	}
}
...
// $LC_ALL / $OK_LANG. Unknown locales return "" so DetectLanguage falls
...
// TestDetectLanguagePriority verifies override beats env and that OK_LANG
...
func TestDetectLanguagePriority(t *testing.T) {
	...
	t.Setenv("OK_LANG", "en")
	if got := DetectLanguage(""); got != "en" {
		t.Errorf("OK_LANG=en overriding LANG=zh: got %q, want en", got)
	}
