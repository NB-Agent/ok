...
func (c *Controller) Compose(text string) string {
	...
	// Environment/diagnosis context is injected every envDiagInterval turns so
	// the cache-stable system-prompt prefix stays byte-identical (warm cache).
	if c.envDiagnosis != "" && c.envDiagInterval > 0 {
		c.mu.Lock()
		turn := c.turn
		c.mu.Unlock()
		if turn%c.envDiagInterval == 0 {
