...
const planApprovedMessage = "Plan approved. Execute now. Track tasks with todo_write (one in_progress at a time), sign off with complete_step and evidence (verification/diff/files)."
...
// approve; on approval it exits plan mode, seeds the task list from the plan, and
// continues straight into execution.
func (c *Controller) runTurn(ctx context.Context, input string) error {
	...
	if c.executor != nil {
		...
		} else if ratio := c.executor.ContextUsage(); ratio >= 0.80 {
			// Soft high-water mark: proactively compact before the turn to
			// keep the prompt fast and avoid cascading timeouts.
			c.sink.Emit(&event.Event{Kind: event.Notice, Level: event.LevelWarn,
				Text: fmt.Sprintf("context high (%.0f%%) — proactively compacting before turn", ratio*100)})
			if err := c.executor.AggressiveCompact(ctx); err != nil {
				// Non-fatal — log and continue; the turn might still work.
				c.sink.Emit(&event.Event{Kind: event.Notice, Level: event.LevelWarn,
					Text: "proactive compact failed: " + err.Error()})
			}
		}
	}
	...
	allow, _, err := c.approval.requestApproval(ctx, PlanApprovalTool, "")
	if err != nil {
		return err
	}
	if !allow {
		c.sink.Emit(&event.Event{Kind: event.Notice, Level: event.LevelInfo,
			Text: "plan rejected — staying in plan mode for revision"})
		return nil // keep planning; plan mode stays on
	}
	c.SetPlanMode(false)
	c.seedPlanTodos(proposal)
	c.approval.SetAutoApprove(true)
	defer c.approval.SetAutoApprove(false)
