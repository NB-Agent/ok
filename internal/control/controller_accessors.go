...
// FirstTurnInject exposes the memory/shared-knowledge/skill-index block that
// rides the first user turn via Compose (not the system prompt), so the
// cache-stable prefix stays byte-identical across sessions.
func (c *Controller) FirstTurnInject() string { return c.firstTurnInject }
...
// Checkpoints returns the list of checkpoint metadata for the active session.
// Returns nil when checkpoints are disabled (CheckpointRoot was empty).
...
// Rewind reverts the workspace to the start of the given turn and restores
// the conversation to that point. Returns written paths, deleted paths, and
// any error. A no-op when checkpoints are disabled.
func (c *Controller) Rewind(fromTurn int) (written, deleted []string, err error) {
	if c.cp == nil {
		return nil, nil, fmt.Errorf("checkpoints unavailable")
	}
	return c.cp.RestoreCode(fromTurn)
}
