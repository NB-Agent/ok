...
// they enter the cache-stable prefix on the next session. Returns the number
// of entries that were promoted.
func (c *Controller) Unfreeze() (int, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.mem == nil {
		return 0, nil
	}
	if err := c.mem.Unfreeze(); err != nil {
		return 0, err
	}
	c.refreshMemoryLocked()
	return 0, nil // exact count not surfaced; Unfreeze is idempotent
}
