...
func runHTTPTransportTest(t *testing.T, sse bool) {
	...
	if !tools[0].ReadOnly() {
		t.Error("readOnlyHint not honored over HTTP")
	}
...
// TestSSETransportInvalidURL proves a misconfigured SSE spec is rejected.
func TestSSETransportInvalidURL(t *testing.T) {
	_, _, err := StartAll(context.Background(), []Spec{{Name: "s", Type: "sse", URL: ""}})
	if err == nil || !strings.Contains(err.Error(), "url is required") {
		t.Fatalf("sse with empty url should error, got %v", err)
	}
}
