...
// misbehaving server can't make us buffer without bound. 16 MiB is generous
// enough for large tool result payloads (MCP tools/list with many tools) while
// still preventing a single response from exhausting process memory.
...
//
// The mutex serializes a request and its response. That means concurrent tool
...
func (t *httpTransport) call(ctx context.Context, method string, params any) (json.RawMessage, error) {
	...
	defer log.Close("plugin http response", resp.Body)
...
func (t *httpTransport) notify(ctx context.Context, method string, params any) error {
	...
	if _, err := io.Copy(io.Discard, io.LimitReader(resp.Body, maxHTTPBody)); err != nil {
		fmt.Fprintf(os.Stderr, "plugin http: drain body: %v\n", err)
	}
	if resp.StatusCode/100 != 2 {
...
func (t *httpTransport) setNotifyHandler(fn func(method string, params json.RawMessage)) {
	t.onNotify = fn
}
