...
// buildExamplePlugin compiles cmd/ok-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "ok-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "github.com/NB-Agent/ok/cmd/ok-plugin-example").CombinedOutput()
	if err != nil {
...
func TestExamplePluginEndToEnd(t *testing.T) {
	...
	if !echo.ReadOnly() || !wc.ReadOnly() {
		t.Errorf("read-only annotation not honored: echo=%v wordcount=%v", echo.ReadOnly(), wc.ReadOnly())
	}
