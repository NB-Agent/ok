...
// allowedCommands is the whitelist of MCP server commands ACP sessions may
// launch. Only bare names (no path separators) that resolve from PATH are
// accepted. Users configure other plugins via ok.toml [[plugins]], not ACP.
var allowedCommands = map[string]bool{
	"ok-plugin-example": true,
	"ok":                true,
	"npx":               true,
	"uvx":               true,
	"node":              true,
...
// `ok acp` command) implements it by reusing setup()'s assembly: a
...
// ends or ctx is canceled. It owns the JSON-RPC connection and the session
// registry; the Factory supplies the kernel wiring. This is the single entry
// point the `ok acp` command calls.
//
...
// session/load can recover it. Distinct from the cli's timestamp-labeled
...
func (s *service) sessionPrompt(ctx context.Context, raw json.RawMessage) (any, error) {
	...
	sess.setCancel(nil)
	// Check cancellation state before calling cancel(), because cancel()
	// makes runCtx.Err() return context.Canceled regardless of why Run failed.
	...
	if err := sess.ctrl.Snapshot(); err != nil {
		fmt.Fprintf(os.Stderr, "acp: session snapshot: %v\n", err)
	}
...
func mcpSpecs(in []MCPServerSpec) []plugin.Spec {
	...
	for _, m := range in {
		// Basic validation: refuse empty or obviously dangerous commands.
		if strings.TrimSpace(m.Command) == "" {
			continue
		}
		if strings.Contains(m.Command, "/") || strings.Contains(m.Command, "\\") {
			continue // path-like command — must be a bare name resolved from PATH
		}
		if !allowedCommands[m.Command] {
			log.Warn("acp: rejecting MCP command not in whitelist", "command", m.Command)
			continue
		}
