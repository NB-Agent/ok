...
// Serve reads inbound frames until the reader ends or ctx is canceled. Each
		...
		go func() {
			...
			defer func() {
				if r := recover(); r != nil {
					...
					fmt.Fprintf(os.Stderr, "acp: panic in serveRequest: %v\n", r)
				}
			}()
		...
		if h := c.notH[in.Method]; h != nil {
			...
			go func() {
				...
				defer func() {
					if r := recover(); r != nil {
						log.Error("goroutine panic", "recover", r)
						fmt.Fprintf(os.Stderr, "acp: panic in notification handler: %v\n", r)
					}
				}()
				h(ctx, in.Params)
			}()
...
func (c *Conn) serveRequest(ctx context.Context, id json.RawMessage, method string, params json.RawMessage) {
	...
	if err := c.write(outbound{JSONRPC: "2.0", ID: id, Result: raw}); err != nil {
		fmt.Fprintf(os.Stderr, "acp: write success response: %v\n", err)
	}
}
...
// canceled, or the connection closes. Satisfies notifier.
...
func (c *Conn) writeError(id json.RawMessage, code int, msg string) {
	if err := c.write(outbound{JSONRPC: "2.0", ID: id, Error: &rpcError{Code: code, Message: msg}}); err != nil {
		fmt.Fprintf(os.Stderr, "acp: write error response (code=%d msg=%s): %v\n", code, msg, err)
	}
}
