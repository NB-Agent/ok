...
// bindAnswer installs the controller's AnswerQuestion callback so AskRequest
// events can be resolved immediately (ACP has no interactive question UI).
...
func (s *updateSink) Emit(e *event.Event) {
	...
	case event.ApprovalRequest:
		...
		go func() {
			...
			s.requestPermission(e.Approval)
		}()

	case event.AskRequest:
		// ACP has no interactive structured-question UI (unlike the chat
		// TUI).  Answer immediately with "no interactive user" so the ask
		// tool returns its autonomous fallback instead of the turn hanging
		// for 30 minutes.
		go func() {
			defer func() {
				if r := recover(); r != nil {
	...
	default:
		// TurnStarted, Message, Usage, Phase, TurnDone, TurnAborted —
		// consumed by other sinks; ACP only forwards interactive events.
	}
}

func (s *updateSink) send(update any) {
	if err := s.conn.Notify("session/update", SessionUpdateParams{SessionID: s.sessionID, Update: update}); err != nil {
		fmt.Fprintf(os.Stderr, "acp: notify session/update: %v\n", err)
	}
}
...
// approve. Any transport failure or a canceled/rejected outcome denies the call,
...
func (s *updateSink) requestPermission(a event.Approval) {
	...
	// Context with a 2-minute timeout: the user has that long to respond to the
	// permission prompt before the request is treated as denied. Conn.Request
	// also unblocks on connection close, so the round-trip can't outlive the
	// wire even without a deeper parent context here.
...
// answerAsk resolves an AskRequest immediately with an empty answer, so the ask
// tool falls through to its autonomous "decide for yourself" path instead of
// the turn hanging for 30 minutes (ACP has no interactive question UI).
func (s *updateSink) answerAsk(a event.Ask) {
	if s.answer == nil {
		return
	}
	s.answer(a.ID, nil) // empty → ask tool returns autonomous fallback
}
