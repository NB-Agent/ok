...
func TestUpdateSinkDropsAndWarns(t *testing.T) {
	...
	c, ok := u["content"].(map[string]any)
	if !ok {
		t.Errorf("warn content not a map = %v", u["content"])
	} else {
		text, _ := c["text"].(string)
		if !strings.Contains(text, "watch out") {
...
func TestUpdateSinkApprovalDenied(t *testing.T) {
	// Both a "canceled" outcome and a transport error must deny the call.
	for _, tc := range []struct {
		name string
		resp func() (json.RawMessage, error)
	}{
		{"canceled", func() (json.RawMessage, error) {
			r, _ := json.Marshal(PermissionRequestResult{Outcome: PermissionOutcome{Outcome: "canceled"}})
			return r, nil
		}},
