...
func (p *scriptedProvider) Stream(ctx context.Context, _ provider.Request) (<-chan provider.Chunk, error) {
	// Respect ctx like a real provider: a canceled turn fails the next step's
...
func TestE2ECancelMidTurn(t *testing.T) {
	...
	case resp := <-promptCh:
		...
		if pr.StopReason != StopCancelled {
			t.Errorf("stopReason = %q, want canceled", pr.StopReason)
		}
	case <-time.After(2 * time.Second):
...
// blockingTool blocks in Execute until released or ctx is canceled, signaling
