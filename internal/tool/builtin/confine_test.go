...
func TestBashSandboxConfinement(t *testing.T) {
	...
	// Windows Job-Object sandbox provides lifecycle management (KILL_ON_JOB_CLOSE)
	// but not filesystem write restrictions — those are enforced by the in-process
	// file-writer confine layer. Skip the write-confinement assertion on Windows.
	if runtime.GOOS == "windows" {
		t.Skip("Windows sandbox provides lifecycle confinement, not filesystem — see WrapProcess")
	}
	home, err := os.UserHomeDir()
	if err != nil {
		t.Skipf("no home dir: %v", err)
	}
	work, err := os.MkdirTemp(home, ".ok-bashsb-*")
	if err != nil {
	...
	outPath := filepath.Join(home, ".ok-bashsb-escape.txt")
