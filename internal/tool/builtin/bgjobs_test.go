...
func TestBackgroundBashWaitAndOutput(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("background bash test uses Unix commands (printf/sleep) not available on Windows cmd")
	}
...
func TestBackgroundKill(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("background bash test uses Unix commands (sleep) not available on Windows cmd")
	}
	m := jobs.NewManager(event.Discard)
	defer m.Close()
	ctx := jobs.WithManager(context.Background(), m)
