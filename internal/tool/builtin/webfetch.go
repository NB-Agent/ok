...
func (webFetch) Description() string {
	return "Fetch a URL and return its text content. HTML is reduced to readable text; JSON/markdown pass through verbatim. Use for docs, APIs, or hosted source files."
}

func (webFetch) Schema() json.RawMessage {
	return json.RawMessage(`{"properties":{"url":{"type":"string"}},"required":["url"],"type":"object"}`)
}
...
func ssrfGuardedClient() *http.Client {
	ssrfGuardedOnce.Do(func() {
		dialer := &net.Dialer{Timeout: webFetchTimeout}
		ssrfGuardedInst = &http.Client{
			...
			Transport: &http.Transport{
				DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
					...
					if len(ips) == 0 {
						return nil, fmt.Errorf("web_fetch: no addresses resolved for %s", host)
					}
					for _, ip := range ips {
...
func (webFetch) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	req.Header.Set("User-Agent", "ok-web-fetch/1.0")
	...
	defer log.Close("webfetch response", resp.Body)
