...
func WriteWAV(w io.Writer, samples []int16, sampleRate int) error {
	...
	// WAV header — binary.Write to bytes.Buffer never fails, but we check
	// defensively to keep errcheck/linters quiet.
	var werr error
	...
	put([]byte("data"))
	put(uint32(len(samples) * 2))
	for _, s := range samples {
		put(s)
	}
	if werr != nil {
		return fmt.Errorf("wav: header write: %w", werr)
	}
	data := buf.Bytes()
	n, err := w.Write(data)
	if err != nil {
