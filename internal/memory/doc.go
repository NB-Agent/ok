// Package memory implements OK's persistent memory. It mirrors Claude
// Code's two-layer model while honoring OK's cache-first architecture:
//
//   - Hierarchical doc memory: AGENTS.md / OK.md files discovered from the
...
// OK.md, AGENTS.md and CLAUDE.md are all recognized. REASONIX.md is kept as
// legacy fallback for projects that used a previous version.
...
var docNames = []string{"OK.md", "REASONIX.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"OK.local.md", "REASONIX.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
...
func loadFrom(dir string, names []string, scope Scope) []Source {
	...
	for _, name := range names {
		...
		// Reject memory files larger than 512 KiB — a file that large is a
		// mistake, and embedding it in every turn's prefix would bloat API calls.
		if len(b) > 512<<10 {
			fmt.Fprintf(os.Stderr, "memory: %s (%d KiB) exceeds limit — skipped\n", path, len(b)>>10)
			continue
		}
...
// everything else is relative to baseDir. Absolute paths and path-traversal
// escapes (../) are rejected — they would leak files outside the project into
// the system prompt.
func resolvePath(p, baseDir string) string {
	...
	// Reject paths that escape baseDir via ../ traversal.
	if !strings.HasPrefix(cleaned, filepath.Clean(baseDir)+string(os.PathSeparator)) && cleaned != filepath.Clean(baseDir) {
		return ""
	}
	return joined
}

// absOf returns the absolute form of p, falling back to filepath.Clean(p) on error.
// The fallback may produce a relative path — callers using it as a map key should
// ensure consistent path forms across the application.
