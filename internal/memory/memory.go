...
// and the auto-memory store. Store, when non-zero, reuses an existing store
// instead of creating a new one — use this to keep the `remember` tool's handle
// consistent across memory refreshes.
type Options struct {
	CWD     string
	UserDir string
...
// (AGENTS.md / CLAUDE.md, in that order); when none exists it
...
func (s *Set) Block() string {
	...
	if idx := strings.TrimSpace(s.Index); idx != "" {
		b.WriteString("\n## Saved memories\n\n")
		b.WriteString("You have previously saved these memories. Read the linked file with read_file when one looks relevant; save new durable facts with the `remember` tool. Use `recall` for cross-session key-value memory (save/search/forget — always in core). For semantic search, activate the knowledge group first (tool-groups) then use `rag`.\n\n")
		// Show only the first 30 index entries to keep the system prompt lean.
		// Full index is available via `recall` (core, always visible), or `rag` / `semantic-search` (activate knowledge group first via tool-groups).
		lines := strings.Split(idx, "\n")
		if len(lines) > 32 { // 30 entries + "# Memory" header + possible empty lines
			for i, line := range lines {
				if i >= 30 {
					b.WriteString("… (use `recall` (core) or activate knowledge group for `rag`/`semantic-search` to search all memories)\n")
					break
				}
				b.WriteString(line)
				b.WriteString("\n")
...
// Unfreeze promotes all unfrozen auto-memory entries into the frozen block so
// they enter the system-prompt prefix on the next session (and become cache-
// stable). It is a no-op when the store is disabled.
func (s *Set) Unfreeze() error { return s.Store.Unfreeze() }
