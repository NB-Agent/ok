...
func TestPermissionMutators(t *testing.T) {
	...
	if err := c.SetModeStyle("YOLO"); err != nil || c.Mode.Default != "yolo" {
		t.Errorf("set mode: err=%v mode=%q", err, c.Mode.Default)
	}
	if err := c.SetModeStyle("nonsense"); err == nil {
	...
	if len(c.Mode.Deny) != 1 {
		t.Errorf("deny list = %v, want one entry", c.Mode.Deny)
	}
...
func TestPluginMutators(t *testing.T) {
	...
	if err := c.UpsertPlugin(PluginEntry{Name: "ex", Command: "ok-plugin-example"}); err != nil {
...
func TestSaveToRoundTrips(t *testing.T) {
	...
	if err := c.SetModeStyle("yolo"); err != nil {
		t.Fatal(err)
	}
	if err := c.AddPermissionRule("deny", "bash(rm -rf*)"); err != nil {
		t.Fatal(err)
	...
	path := filepath.Join(t.TempDir(), "nested", "ok.toml")
	...
	if got.Mode.Default != "yolo" {
		t.Errorf("mode = %q", got.Mode.Default)
	}
	if len(got.Mode.Deny) != 1 || got.Mode.Deny[0] != "bash(rm -rf*)" {
		t.Errorf("deny list = %v", got.Mode.Deny)
	}
