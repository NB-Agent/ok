...
command = "local-bin"
`
	if err := os.WriteFile("ok.toml", []byte(toml), 0o644); err != nil {
		t.Fatal(err)
	}
	mcp := `{ "mcpServers": {
  ...
  "extra":  { "command": "extra-bin" }
	...
	if byName["shared"].Command != "local-bin" || byName["shared"].URL != "" {
		t.Errorf("ok.toml should win the collision, got %+v", byName["shared"])
	}
...
func TestMergeMCPJSONPrecedence(t *testing.T) {
	// ok.toml already declares "shared" (stdio); .mcp.json offers a colliding
	// "shared" (http) plus a fresh "extra". ok.toml must win on the collision;
	// "extra" gets appended.
	...
	if cfg.Plugins[0].Name != "shared" || cfg.Plugins[0].Command != "local-bin" || cfg.Plugins[0].URL != "" {
		t.Errorf("collision not won by ok.toml: %+v", cfg.Plugins[0])
	}
