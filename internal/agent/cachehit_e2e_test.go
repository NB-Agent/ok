...
const systemPrompt = "You are ok, a coding agent. Be concise and follow project conventions. " +
...
func TestCacheHitCollapsesOnCompaction(t *testing.T) {
	...
	// session in.
	a, sink := newAgent(t, srv.URL, mock.tools(), 1500 /*window tok*/, 4 /*recentKeep*/)
	...
	t.Logf("==== hit-rate curve, compaction ON (window=1500 tok, recentKeep=4) ====")
	collapses := 0
	for i, u := range sink.usages {
		r := hitRate(u)
		marker := ""
...
func TestCacheHitSurvivesTooSmallWindow(t *testing.T) {
	...
	a, sink := newAgent(t, srv.URL, mock.tools(), 900, 4)
...
func TestReleaseCacheHitGuard(t *testing.T) {
	if os.Getenv("OK_RELEASE_CACHE_GUARD") == "" {
		t.Skip("set OK_RELEASE_CACHE_GUARD=1 to run the release cache guard")
	}

	threshold := envInt("OK_CACHE_GUARD_THRESHOLD", 90)
	maxLowCases := envInt("OK_CACHE_GUARD_MAX_LOW_CASES", 1)
	...
	if len(lows) > maxLowCases {
		...
		if os.Getenv("OK_CACHE_GUARD_STRICT") != "" {
			t.Fatal(msg)
		}
	}
}
