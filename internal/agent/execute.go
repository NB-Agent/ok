...
func (a *Agent) executeOne(ctx context.Context, call provider.ToolCall) toolOutcome {
	...
	if a.evidenceLedger != nil {
		cctx = evidence.WithLedger(cctx, a.evidenceLedger)
	}
	if a.jobs != nil {
	...
	defer cancel()
	execStart := time.Now()
	// Snapshot the pending change (if the tool supports preview) so
	// checkpoint/rewind has a before-image before the write touches disk.
	if a.onPreEdit != nil && !t.ReadOnly() {
		if pv, ok := t.(tool.Previewer); ok {
			ch, err := pv.Preview(json.RawMessage(call.Arguments))
			if err == nil && ch.Path != "" {
				a.onPreEdit(ch)
			}
		}
	...
	if err != nil {
		...
		if a.evidenceLedger != nil {
			a.evidenceLedger.Record(evidence.ReceiptFromToolCall(call.Name, json.RawMessage(call.Arguments), false, toolReadOnly))
		}
	...
	if a.evidenceLedger != nil {
		a.evidenceLedger.Record(evidence.ReceiptFromToolCall(call.Name, json.RawMessage(call.Arguments), true, toolReadOnly))
	}
