...
// Executor is the interface control.Controller needs from the underlying agent.
// *Agent satisfies it.
type Executor interface {
	...
	SetGate(Gate)
	SetMsgBus(*bus.Bus)
	SetPreEditHook(func(diff.Change))
	CheckContextOverflow() error
	AggressiveCompact(ctx context.Context) error
}
