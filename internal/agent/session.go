...
// SkeletonizeAt skeletonizes tool messages at the given indices, replacing
// their Content with a compressed version that preserves structural info.
// Indices that are out of range or point to non-tool messages are silently
// skipped. This is called after a file re-read to compress stale copies.
func (s *Session) SkeletonizeAt(indices []int, toolName string) {
	...
	for _, idx := range indices {
		...
		if s.Messages[idx].Role != provider.RoleTool {
			continue
		}
		s.Messages[idx].Content = skeletonize(s.Messages[idx].Name, s.Messages[idx].Content)
	}
	s.gen++
}
