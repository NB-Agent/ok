...
// the reference terminal frontend: a headless `ok run` writes to stdout,
...
func (s *TextSink) Emit(e *event.Event) {
	...
	case event.Phase:
		...
		fmt.Fprintf(s.out, "[%s]\n", e.Text)
		s.wroteAnything = true
	default: // unhandled event kind — ignore
	}
}
...
func FormatUsageLine(u *provider.Usage, p *provider.Pricing, d ...*event.CacheDiagnostics) string {
	...
	churn := ""
	if len(d) > 0 && d[0] != nil && d[0].PrefixChanged {
		reasons := strings.Join(d[0].PrefixChangeReasons, "+")
		if reasons == "" {
			reasons = "unknown"
		}
		churn = fmt.Sprintf(" · cache changed: %s", reasons)
	}
