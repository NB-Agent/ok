...
// ResetSessionCache zeros the cumulative counters — called when the session is
// replaced so stale totals don't carry over.
func (t *UsageTracker) ResetSessionCache() {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.sessCacheHit = 0
	t.sessCacheMiss = 0
