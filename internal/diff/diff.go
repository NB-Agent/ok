...
const maxDiffInput = 10 * 1024 * 1024 // 10 MB — prevents OOM on huge inputs
...
func Build(path, oldText, newText string, kind Kind) Change {
	...
	if len(oldText) > maxDiffInput || len(newText) > maxDiffInput {
		c.Diff = "\n# diff: input exceeds 10 MB\n"
		return c
	}
	if oldText == newText {
...
func myers(a, b []string) []op {
	...
	// Guard against O(N²) memory: two large, dissimilar files would store
	// a snapshot of the search frontier at every step, using hundreds of GB.
	// Fall back to a trivial delete‑all + insert‑all diff above 50k lines.
	const maxTotalLines = 50000
...
// fallbackDiff returns a trivial delete-all + insert-all op list for files
// too large for the O(ND) Myers algorithm.  It's a degraded but safe diff.
func fallbackDiff(a, b []string) []op {
	ops := make([]op, 0, len(a)+len(b))
	for _, line := range a {
		ops = append(ops, op{typ: opDelete, line: line})
	}
	for _, line := range b {
		ops = append(ops, op{typ: opInsert, line: line})
	}
	return ops
}
