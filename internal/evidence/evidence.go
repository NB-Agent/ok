// Package evidence provides a per-turn ledger of host-observed tool receipts,
// enabling complete_step to validate that cited evidence actually happened
// before the claim. Receipts stay in memory for a single user turn and are
// passed to tools via context.
package evidence
...
// redirection leave no reader/writer receipt, so the command text naming the
// path is the receipt.
...
// ReceiptFromToolCall builds a receipt from a tool call's name and arguments,
// inferring read/write flags and extracting paths from standard argument keys.
...
// CommandMatches reports whether a cited command string matches a ran command
// by accepting a leading "$ " or "# " prefix (model often copies rendered
// prompt) then checking for an exact match of the significant portion, or
// containment when the cited command is a long multi-command that subsumes a
// shorter single ran command.
func CommandMatches(cited, ran string) bool {
	cited = strings.TrimSpace(cited)
	ran = strings.TrimSpace(ran)
	if cited == ran {
		return true
	}
	// Strip "$ " or "# " prefix that the model sometimes copies from a rendered
	// shell prompt (e.g. "$ go build").
	for _, prefix := range []string{"$ ", "# "} {
		if strings.HasPrefix(cited, prefix) {
			cited = strings.TrimPrefix(cited, prefix)
			cited = strings.TrimSpace(cited)
		}
	}
	if cited == ran {
	...
	// The model might cite "go test ./a ./b" when the single ran command was
	// "go test ./a". Called-included-in-cited is common for multi-package runs;
	// the reverse (model cited a prefix) is not, and would let a cited "go" match
	// an unrelated "go test ./...".
	if strings.Contains(cited, ran) {
		return true
	}
	return false
}
