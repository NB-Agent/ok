...
// Filters out items with empty content — matches the Go backend's behavior
// (todo.go silently skips empty items) so the frontend won't render empty circles.
export function parseTodos(args: string): Todo[] {
  try {
    const a = JSON.parse(args) as { todos?: Todo[] };
    if (!Array.isArray(a.todos)) return [];
    return a.todos.filter((t) => t.content !== "");
  } catch {
    return [];
  }
}
