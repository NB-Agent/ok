...
export interface AppBindings {
  ...
  // Memory panel: read the loaded OK.md hierarchy + saved auto-memories,
  // quick-add a note to a scope's OK.md (≡ "#<note>"), and overwrite a doc
  ...
  SetSandbox(bash: string, network: boolean): Promise<void>;
  SetLanguage(lang: string): Promise<void>;
  SavePlugin(p: PluginView): Promise<void>;
  DeletePlugin(name: string): Promise<void>;
  SetRouter(enabled: boolean, cheap: string, expensive: string): Promise<void>;
  RunningBots(): Promise<BotStatus[]>;
  SetBotEnv(key: string, value: string): Promise<void>;
...
declare global {
  ...
  // Minimal SpeechRecognition for webkit prefix (Chrome/Edge).
...
function makeMockApp(): AppBindings {
  let cancelled = false;
  let cwd = "~/projects/ok"; // mutable so PickWorkspace is visible in dev
  ...
  const settings: SettingsView = {
    ...
    permissions: { mode: "normal", deny: ["bash(rm *)"], allow: [], ask: [] },
    sandbox: { bash: "enforce", network: true },
    plugins: [],
    router: { enabled: false, cheapModel: "", expensiveModel: "" },
    language: "",
    configPath: "~/projects/ok/ok.toml",
  ...
  return {
    ...
    async PickWorkspace() {
      ...
      cwd = cwd.endsWith("another-project") ? "~/projects/ok" : "~/projects/another-project";
      return cwd;
    },
    async ContextUsage() {
    ...
    async Memory() {
      return {
        available: true,
        storeDir: "~/.config/ok/projects/-mock/memory",
        docs: [
          {
            path: "OK.md",
            scope: "project",
            body: "# OK project memory\n\nMock doc shown in the browser dev seam.\n\n## Notes\n\n- prefers concise replies",
          },
          {
            path: "~/.config/ok/OK.md",
            scope: "user",
        ...
        scopes: [
          { scope: "user", path: "~/.config/ok/OK.md" },
          { scope: "project", path: "OK.md" },
          { scope: "local", path: "OK.local.md" },
        ],
      };
    },
    async Remember(scope: string, note: string) {
      emit({ kind: "notice", level: "info", text: `remembered → ${scope}` });
      return `${scope} OK.md (mock): ${note}`;
    },
    ...
    async AddPermissionRule(list: string, rule: string) {
      if (list === "mode") return;
      const arr = (settings.permissions as any)[list];
      if (Array.isArray(arr) && !arr.includes(rule)) arr.push(rule);
    },
    async RemovePermissionRule(list: string, rule: string) {
      if (list === "mode") return;
      const arr = (settings.permissions as any)[list];
      if (Array.isArray(arr)) { (settings.permissions as any)[list] = arr.filter((r: string) => r !== rule); }
    },
    async SetSandbox(bash: string, network: boolean) {
      settings.sandbox = { bash, network };
    },
    async SetLanguage(lang: string) {
      settings.language = lang;
    },
    async SavePlugin(_p: PluginView) {
      // mock: no-op
    },
    async DeletePlugin(_name: string) {
      // mock: no-op
    },
    async SetRouter(_enabled: boolean, _cheap: string, _expensive: string) {
      // mock: no-op
    },
    async RunningBots() {
      return [];
    },
    async SetBotEnv(_key: string, _value: string) {
      // mock: no-op
    },
    async StartBot(_name: string) {
      // mock: no-op
    },
    async StopBot(_name: string) {
      // mock: no-op
    },
