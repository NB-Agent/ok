...
function applyEvent(s: State, e: WireEvent): State {
  ...
  switch (e.kind) {
    ...
    case "message": {
      // The tool_dispatch handler may have already finalized currentAssistant
      // (streaming=false, cleared the ref) when a tool_preparing event arrived
      // mid-stream. Calling ensureAssistant would create a NEW assistant with
      // the same content — the duplication bug. Instead, find the most recent
      // existing assistant item and finalize it.
      ...
      const targetId = s.currentAssistant ?? lastAssistantId(s.items);
      // Extract usage from the message event (the pipe emits ModelFinalEvent as
      // kind:"message" with usage attached; kind:"usage" events never fire in
      // pipe mode, so we capture here).
      ...
      if (targetId) {
        const next = s.items.map((it) =>
          it.kind === "assistant" && it.id === targetId
            ? { ...it, text: e.text ?? it.text, reasoning: e.reasoning ?? it.reasoning, streaming: false }
            : it,
        );
        return { ...s, items: next, currentAssistant: undefined, usage, context: { ...s.context, used }, turnTokens, seq: s.seq };
      }
      // No assistant exists yet (rare: turn with only tool calls, no text).
      // Fall through to ensureAssistant as before.
      const { items, id, seq } = ensureAssistant(s);
      ...
      return { ...s, items: finalized, currentAssistant: undefined, usage, context: { ...s.context, used }, turnTokens, seq };
    }
    ...
    case "tool_dispatch": {
      ...
      // When a new tool call fires and there's a streaming assistant, finalize it
      // and start a fresh assistant after the tool. Without this, text deltas
      // after the tool would keep writing to the assistant that sits *before* the
      // tool card — the "tool running behind output" visual bug.
      let items = s.items;
      let currentAssistant = s.currentAssistant;
      if (currentAssistant) {
        items = items.map((it) =>
          it.kind === "assistant" && it.id === currentAssistant ? { ...it, streaming: false } : it,
        );
      ...
      return { ...s, seq: s.seq + 1, items: [...items, item], currentAssistant };
    }
    ...
    case "notice":
      ...
      return {
        ...
        seq: s.seq + 1,
        items: [...s.items, { kind: "notice", id: `n${s.seq}`, level: (e.level ?? "info") as "info" | "warn", text: e.text ?? "" }],
      };
