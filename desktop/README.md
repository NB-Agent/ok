# OK Desktop (Wails shell)

A native desktop window around the OK Go kernel. The same
...
`desktop/` is its own Go module (`module github.com/NB-Agent/ok/desktop`, `replace
github.com/NB-Agent/ok => ../`). That keeps the CGO + WebKit desktop build entirely
separate from the CLI's `CGO_ENABLED=0` single-static-binary guarantee: the parent
module's `go build / vet / test ./...` skip this directory, while the import path
stays under `ok/` so it can still import the `ok/internal/*` kernel.
...
```sh
cd desktop
wails build          # → build/bin/OK(.app/.exe)
```
