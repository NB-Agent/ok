...
> OK is a coding agent: a thin harness driving multiple models, with **all
...
```
OK/
├── go.mod / go.sum          # module OK; require BurntSushi/toml
├── Makefile                 # build / cross / vet / fmt / test
├── README.md / README.zh-CN.md
├── ok.example.toml         # sample config
├── docs/SPEC.md             # this file
├── cmd/ok/main.go          # entry; blank-imports built-in providers/tools
├── cmd/ok-plugin-example/  # reference MCP stdio plugin (a runnable example)
└── internal/
    ...
    ├── permission/          # per-call deny rules + Gate with optional Approver
    ├── command/             # custom slash commands loaded from .ok/commands/*.md
...
- `cmd/ok-plugin-example` is a runnable reference stdio server (`echo`,
...
Long tasks eventually fill the model's context window. OK manages this with
...
- Compaction summarizes the older middle of the session into a single briefing —
  ...
  summarized away.
- The dropped originals are archived to `~/.config/ok/archive/<timestamp>.jsonl`
...
### 3.7 Mode & denial (`internal/permission`) — interaction style + deny rules

OK uses a simple three-way mode to control writer tools:

```go
// Policy holds deny rules only. Mode decisions are handled by the caller (agent).
type Policy struct { Deny []Rule }
func (p Policy) Denied(toolName string, args json.RawMessage) (string, bool)
```

- **Three modes.** `plan` (read-only, writers blocked), `normal` (prompt before
  writers), `yolo` (writers auto-allowed). The mode is set via `[mode]` in config
  and can also be toggled live with `shift+tab` in the composer.
- **deny rules** are "ToolName" or "ToolName(glob)" patterns that hard-block
  in **every** mode. `deny = ["bash(rm -rf*)"]` blocks `rm -rf /` even in YOLO.
  The subject is extracted generically from the call's JSON args by known keys
  — `command` (bash), `path` / `file_path` (file tools), `pattern` (grep/glob).
- **Approving writers.** In `normal` mode, the interactive front-end prompts the
  user — allow once / always allow / deny — via an `Approver`. A non-interactive
  run (`ok run`, a sub-agent, anything with no TTY) cannot prompt, so it
  auto-allows writers (deny rules still apply). In `plan` mode, *all* writers are
  blocked before reaching the gate; in `yolo` mode, *all* writers are allowed
  (deny rules still apply).
- **Relationship to plan mode.** Plan mode (§3.4) is an orthogonal, coarser gate
  that refuses *all* writers regardless of mode; it is checked first. The
  deny rules are the fine-grained, always-on safety net underneath it.

Out of the box (`default = "normal"`) `ok chat` prompts before each writer/bash
call (`y` once · `a` this session · `n` no). `ok run` stays autonomous but still
honors `deny`. `deny` rules harden all three modes.
...
- **Custom commands** are Markdown files under `.ok/commands/` (project) and
  `~/.config/ok/commands/` (user); the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — OK
...
- Resolution is asynchronous (off the TUI event loop); a fetch failure surfaces
  ...
  — they do **not** pass the deny gate (§3.7).
...
Resolution order: **flag > project `./ok.toml` > user `~/.config/ok/config.toml`
...
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $OK_LANG

[agent]
system_prompt = "You are OK, a coding agent..."  # or system_prompt_file = "..."
max_steps     = 25
temperature   = 0.0
...
[sandbox]
bash    = "enforce"         # OS-level confinement (macOS Seatbelt, Linux Landlock, Windows Job Objects)
network = true              # egress allowed so builds/downloads work
...
`ok setup` writes this default config so the CLI is usable out of the box.
...
`[[plugins]]`; on a name collision `ok.toml` wins (it is the more explicit,
OK-specific source). This lets a server already configured for Claude work in
OK unchanged.
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o ok ./cmd/ok`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  command unconfined via the deny gate (in `ok run`, the command just
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `ok run`.
